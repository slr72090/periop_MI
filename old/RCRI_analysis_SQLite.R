...
## Logit Regression analysis ## ------------------------------------------------------------------------------
data_dispo <- data %>% filter(died == 0)
data$ICF <- as.numeric(data_dispo$ICF==1)
sub.PCA.dispo <- data_dispo[,(colnames(data) %in% c("ICF", "year", "race", "gender", "invasive_mgmt", "PNA", "sepsis", "Afib", "PE", "DVT", "Bleed", "NSTEMI", "severe_MI", unlist(retained_vars)))]
...
data_full_dispo <- data_dispo %>% 
  select(c(year, ICF, invasive_mgmt, died, var_names_full)) # %>% select(-c(RCRI_pt, Ischemic_stroke, los, ind, age_factor, `RCRI > 3`))
out.full.dispo <- glm(ICF ~. + NSTEMI*severe_MI, data = data_full_dispo, family = "binomial")
...
out.RCRIplus.dispo <- glm(as.numeric(ICF) ~  age +
                  ...
                  NSTEMI + 
                    ...
                    DVT + 
                    Bleed, 
                data = data_dispo, family = "binomial")
